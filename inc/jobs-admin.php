...
function wpbb_jobs_handle_seed_demo() {
    ...
    $result = wpbb_jobs_seed_sample_data();
    $message = sprintf( __( 'HR demo data is ready: %1$d companies, %2$d new jobs, %3$d candidate profiles and %4$d new sample applications.', 'wp-bbtheme-child' ), (int) $result['companies'], (int) $result['jobs_created'], (int) $result['resumes'], (int) $result['applications_created'] );
    set_transient( 'wpbb_jobs_setup_notice_' . get_current_user_id(), array( 'message' => $message ), MINUTE_IN_SECONDS );
    wp_safe_redirect( admin_url( 'admin.php?page=wpbb-jobs-setup' ) );
    exit;
}
