...
if ( ! function_exists( 'wpbb_child_v62_profile' ) ) {
    function wpbb_child_v62_profile() {
        ...
        if ( $pack ) $profile = array_replace_recursive( $pack, $profile );
...
if ( ! function_exists( 'wpbb_child_v62_repair_serialized_content' ) ) {
    /**
     * Repair demo serialization without discarding authored spacing/alignment.
     *
     * Only simple demo-owned Group wrappers are converted to BBuilder Div. Any
     * Group carrying anchors, spacing, colour, dimensions or other block-support
     * data is deliberately left as a native block so WordPress can preserve it.
     */
        ...
        $normalize = static function( $items ) use ( &$normalize ) {
            ...
            foreach ( (array) $items as $block ) {
                ...
                if ( 'core/group' === $name ) {
                    ...
                    $safe_keys = array( 'className', 'layout', 'tagName' );
                    $has_complex_supports = (bool) array_diff( $keys, $safe_keys );
                    $is_demo_wrapper = '' !== $class && preg_match( '/(?:^|\s)(?:wp-theme-|wpbb-)/', $class );

                    if ( $is_demo_wrapper && ! $has_complex_supports ) {
                        $tag = sanitize_key( (string) ( $attrs['tagName'] ?? 'div' ) );
                        if ( ! in_array( $tag, array( 'div','section','article','aside','main','header','footer','nav' ), true ) ) $tag = 'div';
                        $block['blockName'] = 'wpbb/bootstrap-div';
                        $block['attrs'] = array(
                            'tagName'        => $tag,
                            'containerClass' => '',
                            'utilityClasses' => $class,
                            'className'      => 'wpbb-demo-wrapper',
                        );
                        $block['innerHTML'] = '';
                        $block['innerContent'] = array_fill( 0, count( (array) ( $block['innerBlocks'] ?? array() ) ), null );
                    }
                }
...
if ( ! function_exists( 'wpbb_child_v62_is_managed_demo_page' ) ) {
    function wpbb_child_v62_is_managed_demo_page( $page_id ) {
        ...
        if ( '1' === (string) get_post_meta( $page_id, '_wp_theme_demo_homepage', true ) ) return true;
        if ( get_post_meta( $page_id, '_wp_theme_demo_profile', true ) ) return true;
...
if ( ! function_exists( 'wpbb_child_v62_rebuild_demo_pages' ) ) {
    /**
     * v3.8.10.80 canonical multilingual demo repair.
     *
    ...
    function wpbb_child_v62_rebuild_demo_pages( $force = false ) {
        ...
        $done_key   = 'wpbb_child_v73_demo_system_' . $stylesheet;
        ...
        // v3.8.10.80 keeps every managed Polylang demo page on the same
        // search-led Jobs structure as English. Parent translation helpers only
        // overwrite translations that Starter Setup owns; editor-created pages
        // remain untouched.
        if ( function_exists( 'wp_theme_demo_polylang_source_ids' ) ) {
            $source_ids = array_merge( $source_ids, (array) wp_theme_demo_polylang_source_ids( 'jobs', $front ) );
        }
        $source_ids = array_values( array_unique( array_filter( array_map( 'absint', $source_ids ) ) ) );
        if ( $source_ids ) {
            wpbb_child_v71_sync_managed_translations( $source_ids );
            if ( function_exists( 'wpbb_jobs_localize_managed_translation_links' ) ) {
                wpbb_jobs_localize_managed_translation_links( $source_ids );
            }
        }
...
if ( ! function_exists( 'wpbb_child_v62_after_demo_import' ) ) {
    function wpbb_child_v62_after_demo_import( $page_id = 0, $profile = array() ) {
        delete_option( 'wpbb_child_v73_demo_system_' . sanitize_key( get_stylesheet() ) );
        // Run after Polylang's initial copy, but serialize English from the same
        // canonical parent content. Managed localized pages are re-synchronised to the same canonical structure.
...
if ( ! function_exists( 'wpbb_child_v62_normalize_on_save' ) ) {
    /**
     * Do not rewrite arbitrary editor content on save. The managed demo repair
     * runs separately and is intentionally limited to demo-owned pages.
     */
...
if ( ! function_exists( 'wpbb_child_v65_enqueue_editor_css' ) ) {
    /** Keep Gutenberg chrome clean: only editor-scoped BBuilder/content rules load in wp-admin. */
